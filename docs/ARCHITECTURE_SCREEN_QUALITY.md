...
| 5 | done | `lib/critic/render.ts` draws any screen to a PNG through the canvas's own SVG and resvg, so the pipeline sees its own output; `lib/critic/critic.ts` shows the PNG and an inventory of the screen's objects to the configured model with the pack's checklist and returns a scored report whose findings name real objects (a finding naming an object the screen does not have is dropped); `/api/critique`; the Validation page reviews a screen and shows the render beside the findings, each clickable to the object. `scripts/critique-demo.mts` renders a sample plant and prints the critic's report. The first three runs against the transfer pump station found: symbols invisible on the grey ground (stroke was scaled with the path), pumps labelled "level" (the headline rule ignored the equipment class), indicator labels under the value (a row too narrow), `TNK_101_LEVEL` and `PMP_102_HRS` unrecognised as level and hours (missing suffixes). All four fixed; the fourth run scores 4/5 with one finding, a pump that has no reading in the tag list |
