...
/** Board units between screens - wide enough to read as separate panels. */
export const BOARD_GAP = 72;
/** Room above each screen for its name. */
export const BOARD_LABEL = 34;
...
export function ScreenBoard({
  ...
  const metrics = useMemo(
    () => boardMetrics(screens.length, size),
    [screens.length, size.width, size.height],
  );
      ...
      {screens.map((screen, index) => {
        ...
        return (
          <div
            ...
            className="absolute"
            style={{
              left: column * metrics.cell.width * scale,
              top: row * metrics.cell.height * scale,
              width: view.Width * scale,
              height: (view.Height + BOARD_LABEL) * scale,
            }}
          >
            {/* The frame's name, above it, the way a Figma frame carries one.
                Clicking it is the least ambiguous way to make a screen live. */}
            <button
              type="button"
              onClick={() => onFocus(screen.UniqueId)}
              className={cn(
                "focus-ring flex w-full items-baseline gap-2 truncate px-0.5 text-left transition",
